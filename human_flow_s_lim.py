...
class Pedestrian(Agent):
    def __init__(self, unique_id, model, movement_type):
        ...
        self.movement_type = movement_type  # "cross_up", or "photograph"
    ...
    def photograph_and_move(self):
        ...
        if self.pos[0] >= self.model.grid.width - 2:
            if self.photographing_time > 0:
                ...
                possible_moves = [pos for pos in next_moves if pos[0] >= self.model.grid.width - 2]  # 右2列内の移動
...
class CrosswalkModel(Model):
    def __init__(self, width, height, N):
        ...
        for i in range(self.num_agents):
            movement_type = self.random.choice(["cross_up", "photograph"])
            agent = Pedestrian(i, self, movement_type)
            self.schedule.add(agent)
            if movement_type == "photograph":
                # 写真撮影者は右2列に配置
                x = self.random.randint(self.grid.width - 2, self.grid.width - 1)
            else:
